...
const convertPdfToImages = async (base64Data: string): Promise<string[]> => {
  try {
...
export async function GET(req: NextRequest) {
    try {
        ...
        for (const pageNum of sortedPageNumbers) {
            ...
            // Process each entity individually (Questions and Answers only, exclude Images)
            for (const entity of sortedEntities) {
                try {
                    console.log(`🔍 Processing ${entity.entityType} from page ${pageNum}...`);

                    if (entity.entityType === "Question" || entity.entityType === "Answer") {
                        // Get bounding box data for this entity
                        const boundingBoxData = getBoundingBox(entity);
                        console.log(`📍 Got bounding box data for ${entity.entityType}:`, boundingBoxData.length, "boxes");
                        
                        // Make premium OCR API call for this specific entity
                        const premiumOcrResult = await makePremiumOcrCall(entity, boundingBoxData, pdfBase64);
                        
                        processedEntities.push({
                            type: entity.entityType,
                            pageNumber: parseInt(pageNum),
                            originalText: entity.content || "",
                            enhancedText: premiumOcrResult.text || entity.content || "",
                            boundingBoxes: boundingBoxData,
                            confidence: premiumOcrResult.confidence || entity.confidence || 1.0,
                            processed: true,
                            premiumOcrUsed: true
                        });
                    ...
                    console.log(`✅ ${entity.entityType} from page ${pageNum} processed successfully`);

                } catch (entityError) {
                    console.error(`❌ Failed to process ${entity.entityType} from page ${pageNum}:`, entityError);
        ...
        // Create final structured object with Questions and Answers processed, Images included but not processed
        const result = {
            processingMethod: "pdf-to-premium-ocr-individual-processing",
        ...
        // Save the result
        const resultPath = "C:\\Users\\rayap\\OneDrive\\Desktop\\tidbHackathon\\apps\\backend\\output\\premium-ocr-individual-result.json";
        await fs.promises.writeFile(resultPath, JSON.stringify(result, null, 2));
        
        console.log(`✅ Premium OCR individual entity processing complete! Results saved to: ${resultPath}`);

        return NextResponse.json(result);
        
    } catch (error: any) {
        console.error("❌ Premium OCR individual processing failed:", error);
        return NextResponse.json({ 
            success: false, 
            error: (error as Error).message,
            stage: 'Premium OCR individual entity processing'
        }, { status: 500 });
    }
}
...
/**
 * Make premium OCR API call for individual entity
 */
